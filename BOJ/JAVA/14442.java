...
/*
 * 기존 벽부수고 이동하기에서 1을 k로 조금만 바꿔주면 되는 문제
 * 로직이 그대로라 쉬웠다.
 */
public class Main {
	static int ans = Integer.MAX_VALUE, n, m, k;
	static int arr[][] = new int[1010][1010];
	static boolean visit[][][] = new boolean[1010][1010][12];
	...
	static void bfs(int sx, int sy)
	{
		...
		while (!q.isEmpty())
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				else if (arr[nx][ny] == 1 && c < k && !visit[nx][ny][c + 1])
				{
